...
const Transactions = ({ user }) => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      try {
        const statusResponse = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/transactions/database/status`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      const url = `${process.env.REACT_APP_API_BASE_URL}/api/transactions${params.toString() ? '?' + params.toString() : ''}`;
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      console.log('Sending request to:', `${process.env.REACT_APP_API_BASE_URL}/api/transactions`);
      console.log('Request data:', requestData);

      const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/transactions`, requestData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleCsvUpload = async (file) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/transactions/upload-csv`, formData, {
        headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/api/transactions/${deletingTransaction.id}`;
